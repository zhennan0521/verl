...
#
# Based on: run_dapo_lora_baseline_1.5b.sh (same config)
# Changes:  model.path → GALI frozen base model
#           model.lora_adapter_path → GALI adapter
#
# Prerequisites: run_gali_prepare_1.5b.sh must have completed successfully.
#
# Usage:
#   bash run_gali_train_1.5b.sh
# =============================================================================
set -xeuo pipefail
...
if [ ! -d "${GALI_BASE_MODEL}" ] || [ ! -d "${GALI_ADAPTER}" ]; then
    echo "ERROR: GALI init not found. Run run_gali_prepare_1.5b.sh first."
    echo "  Expected:"
    echo "    ${GALI_BASE_MODEL}"
    echo "    ${GALI_ADAPTER}"
    exit 1
fi
...
# Paths — use GALI-modified base model, keep same dataset as baseline
